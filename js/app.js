...
function openNewEmployeeForm() {
    ...
    if (safeGet('saveEmployeeBtn'))
        safeGet('saveEmployeeBtn').textContent = 'Save Employee';
    ...
    if (safeGet('reviewsHistory'))
        safeGet('reviewsHistory').innerHTML = '<div class="empty">Save the employee before adding reviews.</div>';
    if (safeGet('incidentHistory'))
        safeGet('incidentHistory').innerHTML = '<div class="empty">Save the employee before adding incident reports.</div>';
    if (safeGet('stayInterviewHistory'))
        safeGet('stayInterviewHistory').innerHTML = '<div class="empty">Save the employee before adding stay interviews.</div>';
    if (safeGet('docHistory'))
        safeGet('docHistory').innerHTML = '<div class="empty">Save the employee before uploading documents.</div>';
    if (safeGet('onboardingChecklist'))
        safeGet('onboardingChecklist').innerHTML = '<div class="empty">Save the employee before loading onboarding tasks.</div>';
    if (safeGet('onboardingSummary'))
...
function resetDrawerForms() {
    ...
    if (safeGet('noteType'))
        safeGet('noteType').value = '';
    if (safeGet('noteText'))
        safeGet('noteText').value = '';
    if (safeGet('disciplineDate'))
        safeGet('disciplineDate').value = todayInputValue();
    if (safeGet('disciplineType'))
        safeGet('disciplineType').value = '';
    if (safeGet('disciplineDescription'))
        safeGet('disciplineDescription').value = '';
    if (safeGet('disciplineAction'))
        safeGet('disciplineAction').value = '';
    if (safeGet('disciplineStatus'))
        safeGet('disciplineStatus').value = 'Open';
    if (safeGet('incidentDate'))
        safeGet('incidentDate').value = todayInputValue();
    if (safeGet('incidentType'))
        safeGet('incidentType').value = '';
    if (safeGet('incidentLocation'))
        safeGet('incidentLocation').value = '';
    if (safeGet('incidentDescription'))
        safeGet('incidentDescription').value = '';
    if (safeGet('incidentFollowUp'))
        safeGet('incidentFollowUp').value = '';
    if (safeGet('incidentStatus'))
        safeGet('incidentStatus').value = 'Open';
    if (safeGet('meetingDate'))
        safeGet('meetingDate').value = todayInputValue();
    if (safeGet('meetingType'))
        safeGet('meetingType').value = '';
    if (safeGet('meetingSubject'))
        safeGet('meetingSubject').value = '';
    if (safeGet('meetingNotes'))
        safeGet('meetingNotes').value = '';
    if (safeGet('reviewDate'))
        safeGet('reviewDate').value = todayInputValue();
    if (safeGet('reviewType'))
        safeGet('reviewType').value = '';
    if (safeGet('reviewAttendance'))
        safeGet('reviewAttendance').value = '';
    if (safeGet('reviewPerformance'))
        safeGet('reviewPerformance').value = '';
    if (safeGet('reviewTeamwork'))
        safeGet('reviewTeamwork').value = '';
    if (safeGet('reviewAttitude'))
        safeGet('reviewAttitude').value = '';
    if (safeGet('reviewReliability'))
        safeGet('reviewReliability').value = '';
    if (safeGet('reviewOverallResult'))
        safeGet('reviewOverallResult').value = '';
    if (safeGet('reviewStrengths'))
        safeGet('reviewStrengths').value = '';
    if (safeGet('reviewImprovements'))
        safeGet('reviewImprovements').value = '';
    if (safeGet('reviewEmployeeComments'))
        safeGet('reviewEmployeeComments').value = '';
    if (safeGet('reviewManagerComments'))
        safeGet('reviewManagerComments').value = '';
    if (safeGet('stayInterviewDate'))
        safeGet('stayInterviewDate').value = todayInputValue();
    if (safeGet('stayInterviewType'))
        safeGet('stayInterviewType').value = '';
    if (safeGet('stayQ1'))
        safeGet('stayQ1').value = '';
    if (safeGet('stayQ2'))
        safeGet('stayQ2').value = '';
    if (safeGet('stayQ3'))
        safeGet('stayQ3').value = '';
    if (safeGet('stayQ4'))
        safeGet('stayQ4').value = '';
    if (safeGet('stayQ5'))
        safeGet('stayQ5').value = '';
    if (safeGet('stayQ6'))
        safeGet('stayQ6').value = '';
    if (safeGet('stayQ7'))
        safeGet('stayQ7').value = '';
    if (safeGet('stayManagerSummary'))
        safeGet('stayManagerSummary').value = '';
    if (safeGet('ecName'))
        safeGet('ecName').value = '';
    if (safeGet('ecRelationship'))
        safeGet('ecRelationship').value = '';
    if (safeGet('ecPhone'))
        safeGet('ecPhone').value = '';
    if (safeGet('ecAltPhone'))
        safeGet('ecAltPhone').value = '';
    if (safeGet('ecNotes'))
        safeGet('ecNotes').value = '';
    if (safeGet('atRiskReasonInput'))
        safeGet('atRiskReasonInput').value = '';
    currentManualAtRiskState = { flagged: false, reason: '' };
    if (safeGet('impactPlayerReasonInput'))
        safeGet('impactPlayerReasonInput').value = '';
    ...
    if (safeGet('saveIncidentBtn'))
        safeGet('saveIncidentBtn').textContent = 'Save Incident Report';
    if (safeGet('saveStayInterviewBtn'))
        safeGet('saveStayInterviewBtn').textContent = 'Save Stay Interview';
    if (safeGet('saveNoteBtn'))
        safeGet('saveNoteBtn').textContent = 'Save Note';
    if (safeGet('saveMeetingBtn'))
...
function populateEmployeeAdminForm(employee) {
    ...
    if (statusSelect) {
        ...
        const existingStatuses = Array.from(statusSelect.options || []).map(option => String(option.value || option.textContent || '').trim().toUpperCase());
...
function applyRolePermissions() {
    ...
    if (supervisorMode) {
        document.querySelectorAll('#deleteEmployeeBtn, #terminateEmployeeBtn, .delete-btn, .danger-delete, [data-admin-only="true"]').forEach(el => {
            el.classList.add('hidden');
            el.disabled = true;
...
function ensureDeleteEmployeeButton() {
    ...
    const findButtonByText = (labels) => {
        ...
        return Array.from(searchRoot.querySelectorAll('button')).find(button => normalizedLabels.includes(String(button.textContent || '').trim().toLowerCase()));
    };
    const newBtn = (drawer && drawer.querySelector("button[onclick='openNewEmployeeForm()']")) ||
        (drawer && drawer.querySelector('#newEmployeeBtn')) ||
        findButtonByText(['New Employee']);
    const saveBtn = (drawer && drawer.querySelector('#saveEmployeeBtn')) ||
        findButtonByText(['Update Employee', 'Save Employee']);
    const actionsRow = (newBtn && newBtn.parentElement) ||
...
async function loadOnboardingTasks(employeeId) {
    ...
    if (!tasks.length) {
        ...
        if (summary)
            summary.textContent = '0 of 0 complete';
        if (bar)
            bar.style.width = '0%';
        return;
    }
    ...
    `).join('');
    if (summary)
        summary.textContent = `${completed} of ${tasks.length} complete`;
    if (bar)
...
async function convertCandidateToEmployee(candidateId) {
    ...
    if (data && data[0]) {
        const normalizedNewEmployee = typeof normalizeEmployee === 'function'
            ...
            : data[0];
        const exists = window.EMPLOYEES.some(e => String(e.id || '') === String(normalizedNewEmployee.id || '') ||
...
function resetCandidateForm() {
    if (safeGet('candidateFirstNameInput'))
        safeGet('candidateFirstNameInput').value = '';
    if (safeGet('candidateLastNameInput'))
        safeGet('candidateLastNameInput').value = '';
    if (safeGet('candidateEmailInput'))
        safeGet('candidateEmailInput').value = '';
    if (safeGet('candidatePhoneInput'))
        safeGet('candidatePhoneInput').value = '';
    if (safeGet('candidatePositionInput'))
        safeGet('candidatePositionInput').value = '';
    if (safeGet('candidateDepartmentInput'))
        safeGet('candidateDepartmentInput').value = '';
    if (safeGet('candidateStageInput'))
        safeGet('candidateStageInput').value = 'Applied';
    if (safeGet('candidateSourceInput'))
        safeGet('candidateSourceInput').value = '';
    if (safeGet('candidateAppliedDateInput'))
        safeGet('candidateAppliedDateInput').value = todayInputValue();
    if (safeGet('candidateNotesInput'))
        safeGet('candidateNotesInput').value = '';
    if (safeGet('candidateNotesPreview'))
        safeGet('candidateNotesPreview').innerHTML = '<div class="empty">Candidate notes will appear here.</div>';
}
...
async function openCandidateDrawer(candidateId) {
    ...
    if (safeGet('candidateEmailInput'))
        safeGet('candidateEmailInput').value = candidate.email || '';
    if (safeGet('candidatePhoneInput'))
        safeGet('candidatePhoneInput').value = candidate.phone || '';
    if (safeGet('candidatePositionInput'))
    ...
    if (safeGet('candidateNotesInput'))
        safeGet('candidateNotesInput').value = candidate.notes || '';
    if (safeGet('candidateNotesPreview')) {
...
async function saveCandidateRecord() {
    ...
    await loadCandidates();
    const refreshedCandidate = CANDIDATES.find(item => String(item.email || '') === String(payload.email || '') &&
...
function clearFilters() {
    if (safeGet('globalSearch'))
        safeGet('globalSearch').value = '';
    if (safeGet('deptFilter'))
        safeGet('deptFilter').value = '';
    if (safeGet('statusFilter'))
        safeGet('statusFilter').value = '';
    currentSort = {
        column: 'name',
        direction: 'asc'
    };
...
function renderKpiEmployeeMetrics() {
    ...
    if (turnoverRiskCard) {
        ...
        if (turnoverRisk >= 40)
            turnoverRiskCard.classList.add('alert');
        else if (turnoverRisk >= 20)
            turnoverRiskCard.classList.add('warn');
        else
            turnoverRiskCard.classList.add('good');
    }
    ...
    try {
        ...
        if (hasAnyData) {
            if (typeof window.updateAtRiskKpi === 'function') {
                ...
                setText('kAtRiskEmployeesSub', atRiskEmployees === 0
...
async function loadReviewDashboard() {
    ...
    try {
        ...
        reviewMeetings.forEach(row => {
            if (!row.employee_id || !row.meeting_date)
                return;
...
function startNewEmployee() {
    ...
    if (details)
        details.innerHTML = '<div class="detail-card"><div class="detail-label">New Record</div><div class="detail-value">Complete the Employee Admin tab to create a new employee.</div></div>';
    safeGet('notesHistory').innerHTML = '<div class="empty">Save the employee before adding notes.</div>';
    safeGet('disciplineHistory').innerHTML = '<div class="empty">Save the employee before adding discipline records.</div>';
    safeGet('meetingsHistory').innerHTML = '<div class="empty">Save the employee before adding meetings.</div>';
    safeGet('ecHistory').innerHTML = '<div class="empty">Save the employee before adding an emergency contact.</div>';
...
async function loadEmergencyContacts(employeeId) {
    ...
    target.querySelectorAll('[data-ec-id]').forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (safeGet('ecRelationship'))
                safeGet('ecRelationship').value = row.relationship || '';
            if (safeGet('ecPhone'))
                safeGet('ecPhone').value = row.phone || '';
            if (safeGet('ecAltPhone'))
                safeGet('ecAltPhone').value = row.alternate_phone || '';
            if (safeGet('ecNotes'))
                safeGet('ecNotes').value = row.notes || '';
            applyRolePermissions();
...
async function loadEmployeeDocuments(employeeId) {
    ...
    const docsWithUrls = await Promise.all(data.map(async (row) => {
...
async function loadEmployeeOnboarding(employeeId) {
    ...
    try {
        ...
        rawRows.forEach(row => {
            ...
            if (seenOnboardingTasks.has(taskKey))
                return;
        ...
        target.innerHTML = Object.entries(grouped).map(([section, items]) => `
            ...
            const overdue = !done && (String(row.status || '').toLowerCase() === 'overdue' ||
...
async function editStayInterview(stayInterviewId) {
    ...
    if (safeGet('stayInterviewDate'))
        safeGet('stayInterviewDate').value = interview.interview_date || todayInputValue();
    if (safeGet('stayInterviewType'))
        safeGet('stayInterviewType').value = interview.interview_type || '';
    if (safeGet('stayQ1'))
        safeGet('stayQ1').value = interview.q1 || '';
    if (safeGet('stayQ2'))
        safeGet('stayQ2').value = interview.q2 || '';
    if (safeGet('stayQ3'))
        safeGet('stayQ3').value = interview.q3 || '';
    if (safeGet('stayQ4'))
        safeGet('stayQ4').value = interview.q4 || '';
    if (safeGet('stayQ5'))
        safeGet('stayQ5').value = interview.q5 || '';
    if (safeGet('stayQ6'))
        safeGet('stayQ6').value = interview.q6 || '';
    if (safeGet('stayQ7'))
        safeGet('stayQ7').value = interview.q7 || '';
    if (safeGet('stayManagerSummary'))
        safeGet('stayManagerSummary').value = interview.manager_summary || '';
    if (safeGet('saveStayInterviewBtn'))
        safeGet('saveStayInterviewBtn').textContent = 'Update Stay Interview';
    ...
    try {
        ...
        target.innerHTML = impactPlayers.map(({ emp, impactMeta }) => {
            ...
            if (impactMeta.manualReason)
                detailParts.push(impactMeta.manualReason);
            if (impactMeta.flaggedDate)
                detailParts.push(String(impactMeta.flaggedDate).slice(0, 10));
