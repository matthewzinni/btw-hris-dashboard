...
declare function todayInputValue(): string;
declare function resetEmployeeForm(): void;
declare function bindDrawerEvents(): void;
...
async function getUserRole(): Promise<UserRole | null> {
    try {
        ...
        if (!accessError && accessRows && accessRows[0]) {
            currentUserAccess = accessRows[0] as UserAccess;
            const accessRole = String(accessRows[0].role || '').toLowerCase().trim() as UserRole;
...
async function loadEmployees(): Promise<Employee[]> {
    try {
        ...
        if (userEmail) {
            ...
            if (!accessError && accessRows && accessRows[0]) {
                currentUserAccess = accessRows[0] as UserAccess;
                currentUserRole = String(accessRows[0].role || currentUserRole || 'user').trim().toLowerCase() as UserRole;
